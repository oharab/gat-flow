...
class ServiceAccountAuth(BaseAuth):
    ...
    def authenticate(
        ...
        if (
            self._delegated_creds
            and self._current_user_email == user_email
            and self._delegated_creds.valid
        ):
...
class AuthManager:
    ...
    @staticmethod
    def create_service_account_auth(
    ...
    @staticmethod
    def auto_detect_auth_type(
        oauth2_file: str = "credentials.json",
        service_account_file: str = "service-account-key.json",
    ) -> BaseAuth:
